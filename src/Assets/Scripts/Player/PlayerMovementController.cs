...
public class PlayerMovementController : MonoBehaviour
{
    ...
    [SerializeField] public GameObject BuyScreen;
    ...
    void Start() {
        ...
        Debug.Log("isAdsOn" + PlayerPrefs.GetInt("isAdsOn", 1));
        if (PlayerPrefs.GetInt("isAdsOn", 1) == 1) {
            Debug.Log("banneracildi");
            HMSAdsKitManager.Instance.ShowBannerAd();
        }
        else if (PlayerPrefs.GetInt("isAdsOn", 1) == 0) {
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if (other.gameObject.tag == "Diamond") {
            ScoreManager.instance.IncreaseDiamond();
    ...
    public void FinishTheGame() {
        ...
        ParticleSystem.transform.position = new Vector3(transform.position.x, transform.position.y + 40f, transform.position.z + 7f);
    ...
    public void RotateCamera(string dir)
    {
        ...
        {
            targetCameraRotation = Quaternion.Euler(25, 0, 0);
            horizontalLimitValue = transform.position.x;
        }
        else if (dir == "right" && currentDirection == "x")
        {
            targetCameraRotation = Quaternion.Euler(25, 90, 0);
